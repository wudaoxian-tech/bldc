...
void foc_hfi_adjust_angle(float ang_err, motor_all_state_t *motor, float dt) { // HFI的PLL
	...
	motor->m_hfi.angle -= dt * (gain_int * ang_err + motor->m_hfi.double_integrator); // ang_err = 估算角度 - 实际角度，故这里是“-”
